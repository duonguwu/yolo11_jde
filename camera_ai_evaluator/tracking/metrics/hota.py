...
class HOTA(_BaseMetric):
    ...
    def eval_sequence(self, data: TrackingMetricInput):
        """
        Calculates the HOTA metrics for one sequence using a standardized input format.

        Args:
            data (TrackingMetricInput): The standardized input data for the metric.
        """
        # Initialise results
        res = {}
        ...
        if data.num_gt_dets == 0:
            res["HOTA_FP"] = data.num_pred_dets * np.ones(
                (len(self.array_labels)), dtype=np.float32
            )
            res["LocA"] = np.ones((len(self.array_labels)), dtype=np.float32)
            res["LocA(0)"] = 1.0
        ...
        tracker_id_count = np.zeros((1, data.num_pred_ids))
